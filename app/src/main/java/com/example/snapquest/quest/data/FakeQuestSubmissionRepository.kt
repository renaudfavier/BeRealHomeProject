...
class FakeQuestSubmissionRepository @Inject constructor(): QuestSubmissionRepository {
    ...
    private val submissionList: MutableList<Submission> = mutableListOf(
    ...
    override suspend fun getAllSubmissions(questId: Int): Result<QuestSubmissionRepository.Response, Error> {
        return Result.Success(submissionList.filter { it.questId == questId }.toResponse())
    }

    override suspend fun getSubmission(id: Int): Result<Submission, Error> {
        submissionList.firstOrNull { it.id == id }?.let { return Result.Success(it) }
    ...
    override suspend fun submit(
        ...
        submissionList.add(
            Submission(
                id = uploadedPhoto.first,
                questId = questId,
                url = uploadedPhoto.second,
                voteCount = 0,
                isMine = true
            )
        )
